...
Up to ~5 generations per run (person + ingredient days need fresh scenes). Poll `job_display` until completed; keep the `rawUrl` values.
  ...
  (c) PRODUCT — do NOT generate. Use the real permanent cutouts on the MIST set via `product_card` / `product_row_card` (soft reflection). If you ever want an AI product scene, img2img from the real cutout only (`medias:[{value:"<product media_id>",role:"image"}]`) with "preserve the bottle's exact silhouette, proportions, frosted glass texture, cap and every printed label character exactly — do not redraw, re-letter or blur any text or logo".
