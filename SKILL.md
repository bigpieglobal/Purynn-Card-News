...
Up to ~5 generations per run (person + ingredient days need fresh scenes). Poll `job_display` until completed; keep the `rawUrl` values.
  ...
  (c) PRODUCT — on PRODUCT-focus days, product cards are styled 연출컷 (staged) scenes, NOT flat cutouts. Generate them img2img from the REAL cutout(s) only (`medias:[{value:"<product media_id>",role:"image"}]`, one hero or a pair per scene) — e.g. the bottle on a wet pale stone ledge or undyed linen with a soft prop, muted dusty-blue tone, soft diffused editorial light, subject lower-right, empty upper-left — always ending with "preserve each bottle's exact silhouette, proportions, frosted glass texture, cap and every printed label character exactly — do not redraw, re-letter, translate, restyle or blur any text or logo. no extra text in the frame". Render the scene with `photo_card` and ALWAYS run the teal snap-back (numpy mask `(b-r>35)&(b>70)&(g>r)`→(73,180,209)). QA each label; if a scene mangles a label, regenerate or fall back to `product_card`/`product_row_card` (real cutouts on the MIST set with reflections). Low-credit fallback (<30 credits): flat cutouts on MIST only.
